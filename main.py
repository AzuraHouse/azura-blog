...
def get_all_posts():
    ...
    # try:
    #     print(f"{current_user.id}"" current user ")
    #     if current_user.id == 1:
...
def login():
    ...
    if request.args.get("error"):
        error = "Email already exist, try log-in!"
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            pass
    return render_template("post.html", post=requested_post, current_user=current_user, form=form, avatar=gravatar)
